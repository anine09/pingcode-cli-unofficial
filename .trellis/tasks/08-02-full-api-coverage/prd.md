...
- [ ] 一条脚本化的端到端链路在真实租户上跑通，且每一跳的关联在 PingCode 网页端可见：
      ...
      缺陷关联到 commit。
      > **实测回填（X3，2026-08-05；G3 收尾批次补注）。** 链路本身**已跑通**（11 跳，见
      > design §D19）。「网页端可见」这半条**没有也无法按字面完成**：本项目只有 API 凭据，没有浏览器
      > 会话。X3 换用了**更强**的证据而不是更弱的：每条 `/v1/relations` 关联都从**对侧端点**读回一行
      > 镜像，且镜像行的 id 与 POST 返回的 id **不同** —— 只有服务端真的物化了双向记录才会如此，
      > 单看「POST 回了 200」推不出（design §D19.5，在 `idea` / `test_case` / `work_item` 三种
      > principal 上各复现一次）。**四跳实质更弱**：branch / PR / build / deploy → work_item 在上游
      > 根本没有按工作项的反向索引（`GET /v1/build/builds` 一个过滤器都没有），只能给自侧
      > `get` 里的 `work_items` 数组，即「服务端存下并在一次往返后重新序列化了该关联」
      > —— 如实标注，不当镜像行算（design §D19.7）。原验收文字保留不改，本注只补账：既不能被悄悄
      > 打勾，也不该被读成未决失败。
